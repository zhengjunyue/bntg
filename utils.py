...
def model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do):
    ...
    for line in model:
        ...
        if operation == "concatenate":
            ...
            #print(inp_out_dict[inp2])
            #inp_out_dict[inp2] = inp_out_dict[inp2].reshape([inp_out_dict[inp2].shape[0]*inp_out_dict[inp2].shape[1],inp_out_dict[inp2].shape[2]])
            inp_dim2 = inp_out_dict[inp2][-1]

            inp_out_dict[out_name] = [inp_dim1 + inp_dim2]
            #inp_dim3 = inp_out_dict[out_name][-1]
            #inp_out_dict[out_name] = torch.nn.Linear(inp_dim3, 40)
...
def forward_model_refac01(
    ...
    for line in model:
        ...
        if operation == "compute":
            ...
            print(outs_dict[out_name].shape)
        elif operation == "dedim":    
            dim1=outs_dict[inp1].shape[-1]
            outs_dict[out_name] = torch.nn.Linear(dim1,int(inp2)).cuda()(outs_dict[out_name]) 
        if to_do == "forward" and out_name == forward_outs[-1]:
                break
        elif operation == "mult":
            ...
            if len(outs_dict[inp1].shape) == 2:
               outs_dict[inp1]=outs_dict[inp1].view(max_len,batch_size,-1)
            print(np.any(np.isnan(outs_dict[inp1].detach().numpy())), np.any(np.isnan(outs_dict[inp2].detach().numpy())))
...
def forward_model(
    ...
    for line in model:
        ...
        if operation == "mse":
            if len(outs_dict[inp1].shape) == 2:
                outs_dict[inp1]=outs_dict[inp1].view(max_len,batch_size,-1)            
            print(np.any(np.isnan(outs_dict[inp1].cpu().detach().numpy())), np.any(np.isnan(outs_dict[inp2].cpu().detach().numpy())))
            outs_dict[out_name] = torch.mean((outs_dict[inp1] - outs_dict[inp2]) ** 2)
            if to_do == "forward" and out_name == forward_outs[-1]:
                break
